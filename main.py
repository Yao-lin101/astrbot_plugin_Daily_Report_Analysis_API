    ...
    async def _summarize_single_group(self, group_id):
        ...
        if provider_id:
            try:
                ...
                self.db.update_group_meta(group_id, last_summarized_id=to_summarize[-1]["id"])
    ...
    async def after_message_sent(self, event: AstrMessageEvent) -> None:
        ...
        if not event.message_obj.group_id:
            ...
            if str(event.get_sender_id()) == specific_user_id:
                self.db.add_private_message(specific_user_id, "bot", reply_text, datetime.now().timestamp())
        ...
        if len(messages) <= 2:
            ...
            if provider_id:
                try:
                    ...
                    raw_result = response.completion_text.strip()
                    # 清理 Markdown 标记
                    if raw_result.startswith("```json"): raw_result = raw_result[7:]
                    elif raw_result.startswith("```"): raw_result = raw_result[3:]
                    if raw_result.endswith("```"): raw_result = raw_result[:-3]
                    ...
                    try:
                        ...
                        logger.error(f"DailyReportAnalysisAPI: 私聊 LLM 返回解析失败: {raw_result}")
                except Exception as e:
